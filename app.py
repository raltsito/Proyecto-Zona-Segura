...
def get_safe_route():
    ...
    if 'features' in ors_data and isinstance(ors_data['features'], list):
    ...
    if not geometries:
        current_app.logger.error(f"No se encontraron geometrías en la respuesta: {ors_data}")
        return jsonify({
            'route': None,
            'safe_zones': [],
    ...
    return jsonify({
        'route': { 'routes': [{ 'geometry': geom } for geom in geometries] },
        'safe_zones': [
